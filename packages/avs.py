"""
>>----------------------------> <---------------------------<<
IMPLEMENTASI PROTOKOL TCP DAN WEBSOCKET PADA LAMPU LALU LINTAS
>>----------------------------> <---------------------------<<
...
@ Mahasiswa
    ...
    Rizky Julianto
"""
import socket, json, time, websocket, thread
import urllib
...
class Server(object):
    def __init__(self, id_server):
        ...
        self.database = {
            ...
            'data': [
                {'response': 'success', 'id_client': 1, 'merah': 0, 'hijau': 0, 'kuning': 0}, # client 1
                {'response': 'success', 'id_client': 2, 'merah': 0, 'hijau': 0, 'kuning': 0}, # client 2
                {'response': 'success', 'id_client': 3, 'merah': 0, 'hijau': 0, 'kuning': 0}, # client 3
                {'response': 'success', 'id_client': 4, 'merah': 0, 'hijau': 0, 'kuning': 0}  # client 4
            ]
        }
    ...
    def handleclient(self, clientsock, address):
        """
        Memeriksa data/status dan mengirimkan data
        lampu merah ke client.
        """
        print "Koneksi dari ", address
        while True:
                try:
                    print "LOG: Menerima data dari client."
    ...
    def handleserver(self, address, port):
        """ Melakukan request ke server utama untuk mengambil data """
        while True:
            try:
                ...
                if data['response'] == 'success':
                    ...
                    #print "LOG: date-modified ", self.database
                    ...
                    if date_serverutama > date_server:
                        print "LOG: Mengupdate data terbaru [date-modified]."
